...
module.exports = function(Config) {
  ...
  module.getLikes = function(location_id, callback) {
    var query = 'SELECT SUM(CASE WHEN (comment = \'\') IS FALSE THEN 1 ELSE 0 END) AS total, SUM(CASE WHEN liked = true THEN 1 ELSE 0 END) AS likes, SUM(CASE WHEN liked = false THEN 1 ELSE 0 END) AS dislikes FROM {table} WHERE location_id = {id} GROUP BY location_id';
